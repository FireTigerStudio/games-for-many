...
具体关键词和数据将在用户填写 `keywords.csv` 后确定；本文件先规定分类方法和决策门槛。
...
`docs/02-keyword-map.md` 在下一批文档中提供正式映射模板。任何关键词在发布前只有一个 `primary_target_url`；次级页面只能作为支持页并用不同意图切分。
