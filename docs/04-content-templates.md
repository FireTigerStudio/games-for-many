...
- 每页 24 款；第一页包含完整分类导语、精选、选择指南、比较模块、相关 Guide 与真实 FAQ；
- 第 2 页以后使用独立 H1（如 `Local 2 Player Games – Page 2`）和简短分页说明，不重复第一页长文；
- 页码、上一页、下一页均为普通链接，各页 self-canonical；
- 不存在的页码返回 404，不重定向到第一页；
- 分类页游戏网格优先于长文，移动端不能先展示大段 SEO 文案；
- URL 参数筛选不形成可索引落地页。

### 2.2 Local 2 Player 第一版内容要求

必须解释同一设备、共享键盘/屏幕、同时操作与轮流操作的区别；列出经过审核的本地游戏；提供按控制方式和对战/合作选择的帮助。不得声称某款支持本地双人，除非试玩或控制说明已经确认。
